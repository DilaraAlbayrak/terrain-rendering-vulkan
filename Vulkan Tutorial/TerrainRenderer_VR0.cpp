...
private:
    ...
    void createLogicalDevice() {
        // --- UNIFIED QUEUE FAMILY SELECTION ---
        // We call findQueueFamilies ONCE and trust its result for everything.
        // This helper function finds a queue family that supports both graphics
        // and presentation to our desktop window surface. This is the most robust way.
        QueueFamilyIndices indices = findQueueFamilies(physicalDevice);

        if (!indices.graphicsFamily.has_value()) {
            throw std::runtime_error("The selected physical device does not have a suitable graphics queue family.");
        }
        if (!indices.presentFamily.has_value()) {
            // This is a critical check for the mirror window.
            throw std::runtime_error("The selected physical device does not have a queue family that can present to the window surface.");
        }
        ...
        // --- Retrieve and enable OpenXR-required device extensions ---
        uint32_t xrDevExtCount = 0;
        xr_check(xrGetVulkanDeviceExtensionsKHR(xrInstance, xrSystemId, 0, &xrDevExtCount, nullptr), "Failed to get device ext count");
        std::vector<char> xrDevExtBuffer(xrDevExtCount);
        xr_check(xrGetVulkanDeviceExtensionsKHR(xrInstance, xrSystemId, xrDevExtCount, &xrDevExtCount, xrDevExtBuffer.data()), "Failed to get device exts");

        std::vector<std::string> xrDevExtStrings;
        {
            std::string xrDevExts(xrDevExtBuffer.data());
            std::stringstream ss(xrDevExts);
            std::string ext;
            while (ss >> ext) {
                xrDevExtStrings.push_back(ext);
            }
        }
        ...
        // --- Obtain queues using the CONSISTENT indices from our helper function ---
        vkGetDeviceQueue(device, indices.graphicsFamily.value(), 0, &graphicsQueue);
        vkGetDeviceQueue(device, indices.presentFamily.value(), 0, &presentQueue);

        // --- Create OpenXR Session using the CONSISTENT graphics queue family index ---
        XrGraphicsBindingVulkanKHR graphicsBinding{ XR_TYPE_GRAPHICS_BINDING_VULKAN_KHR };
        graphicsBinding.instance = instance;
        graphicsBinding.physicalDevice = physicalDevice;
        graphicsBinding.device = device;
        // CRITICAL FIX: We use the graphicsFamily index that we confirmed is suitable for everything.
    ...
    void createRenderPass() {
        // This render pass must be compatible with both the mirror window and the XR swapchains
        VkAttachmentDescription colorAttachment{};
        colorAttachment.format = (VkFormat)xrColorFormat; // Using a fixed format for compatibility
