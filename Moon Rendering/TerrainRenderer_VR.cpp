...
const std::string MODEL_PATH = "models/apollo11-9.obj";
...
private:
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("shaders/multipassVR.vert.spv");
        auto fragShaderCode = readFile("shaders/multipassVR.frag.spv");
    ...
    void loadModel() {
        ...
        for (const auto& shape : shapes) {
            for (const auto& index : shape.mesh.indices) {
                ...
                //vertex.normal = glm::normalize(glm::vec3(glm::rotate(glm::mat4(1.0f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f)) * glm::vec4(originalNormal, 0.0f)));
				//vertex.pos = originalPos;
				vertex.normal = glm::normalize(originalNormal);
